...
test(
  ...
  async () => {
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(mockKeydir, expect.any(Function))
  }
)
...
test(
  ...
  async () => {
    await createKey({ keydir: mockKeydir })
    expect(fs.mkdir).not.toHaveBeenCalledWith(mockKeydir, expect.any(Function))
  }
)
...
test(
  ...
  async () => {
    ...
    expect(createTLSCa).toHaveBeenCalledTimes(1)
  }
)
...
test(
  ...
  async () => {
    ...
    expect(createTLSCa).not.toHaveBeenCalled()
  }
)
...
test(
  ...
  async () => {
    ...
    expect(updateKeyArchiveStream).toHaveBeenCalledWith(
      undefined,
      { '.srl': undefined }
    )
  }
