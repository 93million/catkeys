...
test(
  ...
  async () => {
    ...
    expect(mockPack.entry).toHaveBeenCalledTimes(clientKeyFilenames.length)

    clientKeyFilenames.forEach((filename) => {
      expect(mockPack.entry).toHaveBeenCalledWith(
        { name: filename },
        expect.any(Function)
      )
    })
...
test(
  ...
  async () => {
    ...
    expect(mockPack.entry).toHaveBeenCalledTimes(serverKeyFilenames.length)
