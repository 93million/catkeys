...
test(
  ...
  async () => {
    const updatedFile2 = 'file 2 contents'
    const mockArchiveStream = createArchiveStream({
      file1: 'file 1 contents',
      file2: updatedFile2
    })
    const updatedArchive = await updateKeyArchiveStream(
      mockArchiveStream,
      { file1: 'updated file 1' }
    )

    expect(await expandArchiveStream(updatedArchive))
      .toMatchObject({ file2: updatedFile2 })
  }
)
...
test(
  'should add new items not present in original archive',
  async () => {
    const mockArchiveStream = createArchiveStream({
      file1: 'file 1 contents',
      file2: 'file 2 contents'
    })
    const updatedFile3 = 'file 3 contents'
    const updatedArchive = await updateKeyArchiveStream(
      mockArchiveStream,
      { file3: updatedFile3 }
    )
    ...
    expect(await expandArchiveStream(updatedArchive))
      .toMatchObject({ file3: updatedFile3 })
  }
)
