...
test(
  'should pass catIgnoreMismatchedHostName to checkServerIdentity()',
  async () => {
    const catIgnoreMismatchedHostName = 'testCatIgnoreMismatchedHostNameValue'

    await createHttpsAgent({ catIgnoreMismatchedHostName })

    expect(checkServerIdentity)
      .toBeCalledWith(mockHost, mockPort, { catIgnoreMismatchedHostName })
  }
)
...
test(
  ...
  async () => {
    ...
    expect(createSecureContext).toBeCalledWith(options)
  }
)
...
test(
  ...
  async () => {
    ...
    expect(createSecureContext).toBeCalledWith({})
    expect(https.Agent).toBeCalledWith({
